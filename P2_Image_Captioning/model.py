...
#         sequences = [[[0], 1.0] for _ in range(beam)]
#         seq, score = [], 1.0
#         all_candidates = []
...
#         ## Step 2
#         # Predict the next beam=3 most likely words
#         outputs, hidden = DecoderRNN.get_outputs(self, inputs, hidden)
...
#         best_scores = sorted_score[0][:beam].data
#         best_words = indices[0][:beam].data   
        
# #         inputs = DecoderRNN.get_next_word_input(self, max_indice)
#         for i in range(beam):
#             sequences[i][0].append(best_words[i])
#             sequences[i][1] *= best_scores[i].cpu().item()
#         print("sequences: ", sequences)
        
#         ## Step 3
#         # Predict 3 most likely words out the last 3 words then select 3 best words out of the 9 solutions
#         hidden_states = []
#         step = 3
#         for i in range(beam):
#             print("sequences[i][0][step-2]: ", sequences[i][0][step-2])
#             inputs = DecoderRNN.get_next_word_input(self, sequences[i][0][step-2])
#             print("inputs.shape: ", inputs.shape)
#             outputs, hidden[i] = DecoderRNN.get_outputs(self, inputs, hidden)
#             softmax_score = F.log_softmax(outputs, dim=1) # Define a function to sort the cumulative score
#             sorted_score, indices = torch.sort(-softmax_score, dim=1)
#             print("sorted_score: ", sorted_score)
#             best_scores = sorted_score[0][:beam].data
...
#             softmax_score = F.log_softmax(outputs, dim=1) # Define a function to sort the cumulative score
# #             log_softmax = torch.exp(outputs)
# #             print(log_softmax)
#             print("softmax_score.shape : ", softmax_score.shape)

# #             # Selection beam search
# #             for j in range(softmax_score.shape[1]):
# # #                 print("softmax_score[j]: ", softmax_score[0][j].detach().cpu().numpy()) 
# #                 candidate = [seq + [j], score * softmax_score[0][j]]
...
# #             # Select the beam_width best words
# #             sequences = ordered[:beam]
# #             print("sequences: ", sequences)
            
#             # Another beam attempt
#             # Selection beam search
#             sorted_score, indices = torch.sort(-softmax_score, dim=1)
#             print("sorted_score: ", sorted_score)
#             best_scores = sorted_score[0][:beam].data
#             best_words = indices[0][:beam].data
#             print("best_words: ", best_words, "with scores: ", best_scores)
            
#             ## End beam
                

#             score , max_indice = torch.max(softmax_score, dim=1) # predict the most likely next word, max_indice shape : (1)
#             print("max_indice with softmax: ", max_indice)            
#             print("softmax corresponding score :", score)
...
#         return output
    ...
    ## Beam search implementation of the sample function 
